...
Combines person/helmet detection, ByteTrack, helmet association and line-crossing
into a single ``process_frame`` call that returns detection dicts plus an
annotated frame. One instance runs inside each camera worker process.
"""
...
class SafetyPipeline:
    ...
    def process_frame(
        ...
        """Run the pipeline for a single use-case (``mode``).

        - ``helmet``: run helmet compliance only (no safety lines).
        - ``restricted_area``: run zone/line crossing only (no helmet checks).
        """
        ...
        if getattr(tracked, "tracker_id", None) is not None:
            for idx in range(len(tracked)):
                ...
                if helmet_mode:
                    has_helmet, helmet_conf = self._associator.has_helmet(bbox, helmets)
    ...
    def _annotate(
        ...
        if line_detector is not None:
            for name, p1, p2 in line_detector.segments():
                ...
                cv2.putText(
                    canvas, f"{name.upper()} LINE", (label_x, max(label_y - 8, 14)),
        ...
        for det in detections:
            ...
            if helmet_mode:
                color = (0, 200, 0) if det.helmet_status == HelmetStatus.SAFE else (0, 0, 255)
                label = f"ID:{det.worker_id} {det.helmet_status.value}"
            else:
                color = (0, 0, 255) if det.crossed_lines else (0, 200, 0)
                suffix = ""
                if det.crossed_lines:
                    suffix = f" CROSSED {det.crossed_lines[0].value.upper()}"
                label = f"ID:{det.worker_id}{suffix}"
